...
(defn part-numbers 
  [row idx lines]
  (map parse-long (filter not-empty (mapcat #(str/split % #"\.") (map str/join (kernel row idx lines))))))
  ...
  (let [lines (str/split (slurp file) #"\n")
    ...
    (reduce + (flatten
                (for [row (range (count lines)) 
                  ...
                  (mapcat #(part-numbers row % lines) (get machine-parts row)))))))

(defn solve2
  [file]
  (let [lines (str/split (slurp file) #"\n")
        machine-parts (vec (map #(re-seq-pos #"[*]" %) lines))]
    (reduce + (flatten (map (fn [numbers] (map #(reduce * %) numbers))
                            (filter not-empty 
                                    (for [row (range (count lines)) 
                                          :when (seq? (get machine-parts row))]
                                      (filter #(= 2 (count %))
                                              (map #(part-numbers row % lines) (get machine-parts row)))))))))))))
